...
const SEED_USER_ID = "user_2gKIk8aRchNZBdZTihoixtSZAGE"
...
const generateTransactionForDay = (day: Date) => {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    const isExpense = Math.random() > 0.6 // 60% chance of an expense
    const amount = generateRandomAmount(category)
    const formattedAmount = convertAmountToMiliunits(isExpense ? -amount : amount) // negative amounts are expenses
